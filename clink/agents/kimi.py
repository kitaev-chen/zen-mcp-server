...
class KimiAgent(BaseCLIAgent):
    """Kimi CLI agent with stream-json parsing support.
    
    Kimi requires the prompt to be passed via --command argument,
    ...
    async def run(
        ...
        *,
        ...
        """Override run to pass prompt via -c argument instead of stdin."""
        _ = (files, images, system_prompt)  # Unused for kimi
        
        # Build base command
        command = self._build_command(role=role, system_prompt=None)
        
        # Add prompt via --command argument (not -c)
        command.extend(["--command", prompt])
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                *command,
                stdin=asyncio.subprocess.DEVNULL,  # kimi doesn't use stdin
    ...
    def _recover_from_error(
        ...
        *,
        ...
        """Kimi may still output JSONL events even with non-zero exit codes.
        
        Try to parse stdout to extract any assistant messages.
        """
        try:
